...
const UserInfo = {
  ...
  findDataByAccount(account) {
    const _sql =
      "select id,github_id,account,github from user_info where account = ?";
    return query(_sql, account);
  },
  ...
  getUserInfoById(id) {
    const _sql =
      "select id,github_id,account,github from user_info where id = ?";
  ...
  isFriend(user_id, from_user) {
    const _sql =
      "SELECT  * FROM user_relation  AS u WHERE  u.user_id = ? AND u.from_user = ? ";
    return query(_sql, [user_id, from_user]);
  },
  // 两边都互加为好友
  addFriendEachOther(user_id, from_user, time) {
    const _sql =
      "INSERT INTO user_relation(user_id,from_user,time) VALUES (?,?,?), (?,?,?)";
